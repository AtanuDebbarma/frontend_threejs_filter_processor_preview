...
import type {FilterItem, FilterParams} from '../types/filterTypes';
...
export function normalizeForExport({
  ...
  const normalized = Object.entries(currentEditorValues).map(
    ([index, editor]) => {
      const merged = {...activeFilter.params, ...editor.value};
