...
} from '@/features/post/helpers/text/textPillStyle';
import {
  createCanvasTextMeasurer,
  mapCaretAfterStoryTextWrap,
  wrapStoryTextContentToStage,
} from '@/features/post/helpers/text/textLineWrap';
...
import {normalizeContentForInactive} from '@/features/post/components/menus/text/storyTextPillLines';
import React, {
  forwardRef,
  useCallback,
  useEffect,
...
export const StoryTextPill = forwardRef<StoryTextPillHandle, Props>(
  function StoryTextPill(
    ...
    // Only clear focus when deselected — do not auto-open keyboard on active (breaks drag).
    useEffect(() => {
      if (!isActive) {
        setIsTextFocused(false);
      }
